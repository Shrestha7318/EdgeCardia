 ...
 * I2C transport layer
 *
 * Uses i2c_mem_write() from i2c.c (bare-metal register-level I2C1).
 * I2C1 must be initialized via i2c_init() before calling ssd1306_Init().
 * ------------------------------------------------------------------ */
...
void ssd1306_WriteCommand(uint8_t byte) {
    I2C1_Start();
    I2C1_SendAddress(SSD1306_I2C_ADDR);  // 0x78
    I2C1_SendData(0x00);                  // control byte: command
    I2C1_SendData(byte);
    I2C1_Stop();
}
...
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
    I2C1_Start();
    I2C1_SendAddress(SSD1306_I2C_ADDR);  // 0x78
    I2C1_SendData(0x40);                  // control byte: data
...
void ssd1306_Init(void) {
    ...
    // Simple busy-wait (~100ms at 180MHz). Replace with your own delay_ms() if available.
    for (volatile uint32_t i = 0; i < 1800000; i++);
